...
The current concrete example is `BasicAuth`, which expects HTTP Basic Auth credentials (token ID and secret) on protected operations such as `POST /v1/media/upload-url`.
...
- Plug in godec-specific authentication logic for `BasicAuth`.
...
1. [internal/api/spec.yaml](../internal/api/spec.yaml) defines the OpenAPI contract, including `components.securitySchemes.BasicAuth`.
...
5. If the operation requires `BasicAuth`, the validator calls `middleware.APIKeyAuthenticator(...)`.
6. `middleware.APIKeyAuthenticator(...)` extracts token ID and secret from the HTTP Basic Auth header, validates them through the API key service adapter, and stores the validated key in the request context.
7. If validation succeeds, Echo continues to the generated strict handler and the feature handler.

For the current setup, `BasicAuth` is the only security scheme wired into runtime auth.
...
- **Header mismatch**: the spec and validator disagree on the credential source, such as HTTP Basic Auth vs a custom header.
...
If you are working on the current API key flow, the quickest sanity check is: `BasicAuth` in the spec, HTTP Basic Auth header with `tokenId:secret` in the request, and `middleware.APIKeyAuthenticator(...)` in the validator options.
